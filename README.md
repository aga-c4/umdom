...
docker-compose up
```
7. Для работы бота необходимы следующие компоненты:
- Zigbee2MQTT (через него подключаются Zigbee коммутатор и ведется трансляция из сети Zigbee в MQTT)
- MQTT брокер mosquitto
- keydb (аналог Redis) - в нем содержатся текущие состояния устройств, изменяемые скриптом mqttsubscr.sh
Подробно про установку всего вышеперечисленного читайте в [docs/install.txt](docs/install.txt)
